...
{
	public class UserSavingsGroupModel : PageModel
    {
        ...
        public UserSavingsGroupModel(ApiProcessor apiCall, IConfiguration config, ManageSignIn signInMgm, Logger logger)
        {
            _apiCalls = apiCall;
            _config = config;
        ...
        public async Task<IActionResult> OnGetAsync()
        {
            try
            {
                if (_signInMgm.UserName != null && (!string.IsNullOrEmpty(_signInMgm.UserName.ToString())))
                {
                    ...
                    var _url = _config.GetSection("apiUrls")["getUserSavingsGroupUrl"];
                    string data = JsonConvert.SerializeObject(new GetUserSavingsGroupDTO() { UserId = _signInMgm.UserId });
                    string token = HttpContext.Session.GetString("accessToken");
                    var savingsGroupListResp = await _apiCalls.PostDataAsync(data, _url, token);
                    ...
                    var _respVal = JsonConvert.DeserializeObject<Response<SavingsGroupDTO>>(savingsGroupListResp);
                    if (_respVal == null) { _logger.WriteErrorLog($"SAVINGS GROUP RESPONSE IS NULL"); TempData["ErrorMessage"] = errorMsg; return Page(); }
                    ...
                    savingsGroups.Add(_respVal.Data);
                    return Page();
                }
