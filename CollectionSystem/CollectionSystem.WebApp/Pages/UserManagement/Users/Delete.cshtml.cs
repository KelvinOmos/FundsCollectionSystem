...
namespace CollectionSystem.WebApp.Pages.UserManagement.Users
{
    public class DeleteModel : PageModel
    {
        ...
        public DeleteModel(ApiProcessor apiCalls, IConfiguration config, ManageSignIn signInMgm, Logger logger)
        {
            _apiCalls = apiCalls;
            _config = config;
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            try
            {
                ...
                {
                    string errorMsg = string.Empty;
                    var _url = _config.GetSection("apiUrls")["deleteUserUrl"];
                    string data = JsonConvert.SerializeObject(new deleteUserDTO() { Id = user.Id});
                    string token = _signInMgm.AccessToken;
                    var userResp = await _apiCalls.PostDataAsync(data, _url, token);
                    if (string.IsNullOrWhiteSpace(userResp))
                    {
